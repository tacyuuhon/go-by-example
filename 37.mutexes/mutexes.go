	...
	for r := 0; r < 100; r++ {
		go func() {
			...
			for {
				...
				// 每次循环读取，我们使用一个键来进行访问， Lock() 这个 mutex 来确保对 state 的 独占访问，读取选定的键的值，Unlock() 这个 mutex，并且 ops 值加 1。
				key := rand.Intn(5)
				mutex.Lock()
				total += state[key]
				mutex.Unlock()
				...
				// 这个释放一般是自动处理的，像例如每个通道操作后或者 time.Sleep 的阻塞调用后 相似，但是在这个例子中我们需要手动的处理。
				runtime.Gosched()
			}
		}()
	}
...
/*
	ops: 3527938
	state: map[0:98 1:72 2:85 3:4 4:13]
*/
